...
export const fixIt = props => {
	const { currentBlockId, isPasting, authorizedRuleSettings } = props
	
	// Get the regex of all rules
	let theRegs = rules.filter( reg => true === authorizedRuleSettings?.find( s => s.slug === reg.slug )?.value )
...
export const fixAll = props => {
	...
	const { authorizedRuleSettings } = props
